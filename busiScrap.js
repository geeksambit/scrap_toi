    ...
    return new Promise((resolve,reject) => {
        ...
        response,html)=>{
            if (!error && response.statusCode == 200){
                ...
                let res = await insertIntoDb("business",myObj) ;
                resolve(res)
            }else {
                reject(error)
            }
...
request("https://timesofindia.indiatimes.com/business",(error,
response,html)=>{
    if (!error && response.statusCode == 200){
